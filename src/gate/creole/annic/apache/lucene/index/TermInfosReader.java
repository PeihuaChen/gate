...
final class TermInfosReader {
  ...
  private final int getIndexOffset(Term term) throws IOException {
    ...
    while (hi >= lo) {
      int mid = (lo + hi) >> 1;
      int delta = term.indexCompareTo(indexTerms[mid]);
      if (delta < 0)
	hi = mid - 1;
      else if (delta > 0)
	lo = mid + 1;
  ...
  TermInfo get(Term term) throws IOException {
	...
	&& ((enumerator.prev != null && term.indexCompareTo(enumerator.prev) > 0)
	    || term.indexCompareTo(enumerator.term()) >= 0)) {
      int enumOffset = (int)(enumerator.position/enumerator.indexInterval)+1;
      if (indexTerms.length == enumOffset	  // but before end of block
	  || term.indexCompareTo(indexTerms[enumOffset]) < 0)
  ...
  private final TermInfo scanEnum(Term term) throws IOException {
    SegmentTermEnum enumerator = getEnum();
    while (term.indexCompareTo(enumerator.term()) > 0 && enumerator.next()) {}
    if (enumerator.term() != null && term.indexCompareTo(enumerator.term()) == 0)
  ...
  final long getPosition(Term term) throws IOException {
    ...
    if (term.indexCompareTo(enumerator.term()) == 0)
