...
public class PropertyImpl extends OntologyResourceImpl implements Property {
	...
	/**
	 * The set of domain restrictions (i.e. {@link OClass} objects} set as
	 * domain directly for this property.
	 */
	protected Set directDomain;

	/**
	 * The set of range restrictions (i.e. {@link OClass} or {@link Class}
	 * objects) for this property. This is composed from the
	 * {@link #directRange} plus all the range restrictions from the
	 * super-properties. Once calculated this value is cached.
	 */
	protected Set range;
	 ...
	 *            {@link OClass} or {@link Class} objects depending on the types
	 *            of the values that are permitted.
	 * @param ontology
	...
	public Set getSamePropertyAs() {
		if (this.samePropertiesSet.isEmpty()
			...
			Property propDefinition = this.getOntology()
					.getPropertyDefinitionByName(this.name);
	...
	protected void recalculateDomain() {
		...
		Iterator superPropIter = getSuperProperties(TRANSITIVE_CLOSURE)
				.iterator();
	...
	/**
	 * Returns the set of domain classes for this property. This is composed
	 * from the classes declared as domain restriction for this property plus
	 * all the domain restrictions from the super-properties.
	 */
	 ...
	 * If the value is an {@link OInstance} then, in order to be a valid domain
	 * value it needs to be a member of <b>all</b> the classes defined as
	 * members of the domain of this property. The domain of this property is
	 * defined recursively based on its super-properties as well. For any other
	 * types of values it returns <tt>true</tt>.
	 * 
	 * @param instance
	 *            the instance to be checked.
	 * @return <tt>true</tt> if the provided instance can be a domain value
	 *         for this property.
	 */
	...
	public boolean isValidRange(Object value) {
		if (value instanceof OInstance) {
			...
			while (rangIter.hasNext()) {
				if (!((Class) rangIter.next()).isAssignableFrom(value
						.getClass()))
					return false;
			}
			return true;
	...
	protected void calculateSuperPropertiesClosure() {
		...
		while (properties.size() > 0) {
			...
			if (superPropertiesTransitiveClosure.add(aSuperProperty)) {
				...
				properties.addAll(aSuperProperty
						.getSuperProperties(DIRECT_CLOSURE));
			}
		}
	}
	...
	protected void calculateSubPropertiesClosure() {
		...
		while (properties.size() > 0) {
			...
			if (subPropertiesTransitiveClosure.add(aSuperProperty)) {
				// this super property hasn't been seen before
				properties.addAll(aSuperProperty
						.getSubProperties(DIRECT_CLOSURE));
			}
		}
	}
}
