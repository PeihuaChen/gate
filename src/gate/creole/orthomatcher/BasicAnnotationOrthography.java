...
public class BasicAnnotationOrthography implements AnnotationOrthography {
  ...
  public Annotation updateMatches(Annotation newAnnot, String annotString,HashMap processedAnnots,AnnotationSet nameAllAnnots,List matchesDocFeature) {
    ...
    while (iter.hasNext()) {
      ...
      if (annotString.equals(oldString)) {
        ...
        if (tempAnnot == null) {
          log.warn("Orthomatcher: TempAnnot is null when looking at " + annotString
                  + " | " + oldString + " | old id: " + id);
          return null;
        }
    ...
    newAnnot.getFeatures().put(OrthoMatcher.ANNOTATION_COREF_FEATURE_NAME, matchesList);
    return matchedAnnot;
  }
  ...
  public void updateMatches(Annotation newAnnot, Annotation prevAnnot,List matchesDocFeature,AnnotationSet nameAllAnnots) {
    ...
    get(OrthoMatcher.ANNOTATION_COREF_FEATURE_NAME);
    if ((matchesList == null) || matchesList.isEmpty()) {
      //no previous matches, so need to add
      if (matchesList == null) {
        matchesList = new ArrayList();
        prevAnnot.getFeatures().put(OrthoMatcher.ANNOTATION_COREF_FEATURE_NAME, matchesList);
        matchesDocFeature.add(matchesList);
      }//if
    ...
    if (prevAnnot.getType().equals(this.personType)) {
      String prevGender =
        (String) prevAnnot.getFeatures().get(OrthoMatcher.PERSON_GENDER_FEATURE_NAME);
      String newGender =
        (String) newAnnot.getFeatures().get(OrthoMatcher.PERSON_GENDER_FEATURE_NAME);
      ...
      if (unknownPrevGender && !unknownNewGender)
        prevAnnot.getFeatures().put(OrthoMatcher.PERSON_GENDER_FEATURE_NAME, newGender);
      else if (unknownNewGender && !unknownPrevGender)
        newAnnot.getFeatures().put(OrthoMatcher.PERSON_GENDER_FEATURE_NAME, prevGender);
    }//if
  }
  ...
  public HashSet buildTables(AnnotationSet nameAllAnnots) {
    ...
    if (!extLists) {
      ...
      tempMap.clear();
      tempMap.put(gate.creole.ANNIEConstants.LOOKUP_MAJOR_TYPE_FEATURE_NAME, "cdg");
  ...
  protected Map<String,HashSet<String>> initNicknames(
    ...
    while ((lineRead = reader.readLine()) != null){
      ...
      if (round2Places(Double.valueOf(nickNameLine.get(2))) < OrthoMatcherHelper.round2Places(minimumNicknameLikelihood)) {
        continue;
      }
