...
/**This component is responsible for doing lists lookup. The implementaion is
  ...
  *one entry per line.
  *When this gazetter will be run over some input text (a Gate document) it will
  *generate annotations of type Lookup having the attributes specified in the
  *definition file.
  */
public class DefaultGazetteer implements Runnable{
  ...
  */
         ...
         throws IOException, GazetteerException
  {
    ...
    reader = new InputStreamReader(Files.getResourceAsStream(resPath + resourceName));
  }//public DefaultGazeteer(String resourcePath, String resourceName)


  /**Does the actual loading and prsing of the lists. This method must be called
    *before the gazetteer can be used
    */
...
//Out.println(text + "|" + lookup.majorType + "|" + lookup.minorType);
  ...
  /**The method that does the actual input. This method should never be called
    ...
    */
  public void run(){
    fireStatusChangedEvent("Doing lookup in " + doc.getSourceUrl().getFile() + "...");
...
/*
  public static void main(String[] args){
    try{
      DefaultGazeteer dg = new DefaultGazeteer("d:/tmp/gaztest/extension/lists.def");
      gate.fsm.TestFSM.showGraph("Tokeniser graph (deterministic)", dg.getFSMgml());

      Document doc = Factory.newDocument(new URL("file:///d:/tmp/gaztest/extension/long.lst"));
