 ...
 *
 *  Angel Kirilov (mod AnnotDiffDialog), 22/Aug/2002
 *
 *  $Id$
 *
 */
...
/** This class wraps the {@link gate.annotation.CorpusAnnotationDiff} one. It adds the
  ...
  * adds the CorpusAnnotationDiff as a tool in GATE.
  */
class CorpusAnnotDiffDialog extends JFrame {
  ...
  CorpusAnnotDiffDialog thisAnnotDiffDialog = null;
  ...
  CorpusAnnotationDiff annotDiff = null;
  ...
  /** Constructs an annotDiffDialog object having as parent aMainFrame
    ...
    */
  public CorpusAnnotDiffDialog(MainFrame aMainFrame){
  ...
  public void initLocalData(){
    annotDiff = new CorpusAnnotationDiff();
  ...
  public void initGuiComponents(){
    ...
    // init its label
    keyLabel = new JLabel("Select the KEY corpus");
    ...
    // init its label
    responseLabel = new JLabel("Select the RESPONSE corpus");
  ...
  private void doDiff(){
    try{
      ...
      CorpusAnnotationDiff.weight = d.doubleValue();
  ...
  private void collectSomeFeatures(){
    ...
    gate.Corpus keyCorpus = null;
    keyCorpus = (gate.Corpus) corpusMap.get(
    ...
    if (keyDocAnnotSetComboBox.getSelectedItem()== null ||
      ...
      for(int i=0; i<keyCorpus.size(); ++i) {
        ...
        tempSet = doc.getAnnotations();
        if(null == keySet) keySet = tempSet;
        else keySet.addAll(tempSet);
      } // for
    }
    else{
  ...
  private void initAnnotTypesFalsePoz(){
    gate.Corpus responseCorpus = null;
    responseCorpus = (gate.Corpus) corpusMap.get(
    ...
    if (responseDocAnnotSetFalsePozComboBox.getSelectedItem() == null ||
      ...
      for(int i=0; i<responseCorpus.size(); ++i) {
        ...
        docTypeSet = doc.getAnnotations().getAllTypes();
  ...
  private void initAnnotTypes(){
    ...
    if (keyCorpus == null || responseCorpus == null
        || 0 == keyCorpus.size() || 0 == responseCorpus.size()){
      // init types map with Type,AnnotationSchema pairs
      typesMap.put("No annot.",null);
  ...
  private void initKeyAnnotSetNames(){
    ...
    if (keyCorpus != null && 0 != keyCorpus.size()){
      ...
      for(int i=0; i<keyCorpus.size(); ++i) {
        doc = (Document) keyCorpus.get(i);
        namedAnnotSets = doc.getNamedAnnotationSets();
  ...
  public gate.Corpus getSelectedKeyCorpus(){
    return (gate.Corpus) corpusMap.get(
                                (String)keyDocComboBox.getSelectedItem());
  }//getSelectedKeyCorpus

  /** It returns the selected RESPONSE gate.Document*/
  public gate.Corpus getSelectedResponseCorpus(){
    return (gate.Corpus) corpusMap.get(
                                (String)responseDocComboBox.getSelectedItem());
  }//getSelectedResponseCorpus
  ...
  class DiffRunner implements Runnable{
    ...
    /** This method is overridden in order to implement the needed behaviour*/
    public void run(){
      annotDiff.setKeyCorpus(thisAnnotDiffDialog.getSelectedKeyCorpus());
      annotDiff.setResponseCorpus(
                          thisAnnotDiffDialog.getSelectedResponseCorpus());
