			...
			public void actionPerformed(ActionEvent actionevent) {
				...
				for (int j = 0; j < selectedNodes.size(); j++) {
					...
					if (defaultmutabletreenode.getUserObject() instanceof TClass)
						arraylist.add(((TClass) defaultmutabletreenode
		...
		rangeB.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionevent) {
				if (objectProperty.isSelected()) {
					...
					dataTypeRange = JOptionPane.showInputDialog(null,
							dataTypeRange, "range Class", 3);
				}
			}
	...
	public void actionPerformed(ActionEvent actionevent) {
		...
		if (i == 0) {
			...
			if (ontology.getPropertyDefinitionByName(propertyName.getText()) != null) {
				JOptionPane.showMessageDialog(null, (new StringBuilder())
						.append("Property :").append(propertyName.getText())
			...
			for (int j = 0; j < as.length; j++) {
				OClass oclass = (OClass) ontology
						.getClassByName((String) as[j]);
				if (oclass == null) {
					JOptionPane.showMessageDialog(null, (new StringBuilder())
							.append("Invalid Domain :").append((String) as[j])
							.toString());
					return;
				}
				hashset.add(oclass);
			...
			if (datatypeProperty.isSelected()) {
				...
				try {
					...
					JOptionPane.showMessageDialog(null, (new StringBuilder())
							.append("Invalid Range :").append(dataTypeRange)
							.toString());
					return;
				}
				DatatypePropertyImpl datatypepropertyimpl = new DatatypePropertyImpl(
						propertyName.getText(), comment.getText(), hashset,
						class1, ontology);
				...
				for (int k = 0; k < as1.length; k++) {
					OClass oclass1 = (OClass) ontology
							.getClassByName((String) as1[k]);
					if (oclass1 == null) {
						JOptionPane.showMessageDialog(null,
								(new StringBuilder()).append("Invalid Range :")
										.append((String) as1[k]).toString());
						return;
					}
