...
final class DocumentWriter {
    ...
    while (fields.hasMoreElements()) {
      ...
      if (field.isIndexed()) {
        if (!field.isTokenized()) {		  // un-tokenized field
          addPosition(fieldName, field.stringValue(), "Field"/*, 1*/, position++);
          length++;
        } else {
          ...
          try {
            for (Token t = stream.next(); t != null; t = stream.next()) {
              ...
              if(t.type() == null)
                addPosition(fieldName, t.termText(), "*"/*, t.getPositionIncrement()*/, position++);
              else
                addPosition(fieldName, t.termText(), t.type()/*, t.getPositionIncrement()*/, position++);
  ...
  private final Term termBuffer = new Term("", "", "")/*, 0)*/; // avoid consing

  private final void addPosition(String field, String text, String type/*, int posIncrement*/, int position) {

    termBuffer.set(field, text, type/*, posIncrement*/);
    ...
    if (ti != null) {				  // word seen before
      ...
      Term term = new Term(field, text, type/*, posIncrement*/, false);
    ...
    try {
      ...
      for (int i = 0; i < postings.length; i++) {
        ...
        if (termVectorWriter != null && termVectorWriter.isFieldOpen()) {
          /* Niraj */ termVectorWriter.addTerm(posting.term.text(), postingFreq);
          //termVerctorWriter.addTerm(posting.term,postingFreq);
        }
      }
      if (termVectorWriter != null)
