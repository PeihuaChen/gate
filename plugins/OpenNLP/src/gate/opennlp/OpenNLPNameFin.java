...
class OpenNLPNameFin extends AbstractLanguageAnalyser {
	...
	public void execute() throws ExecutionException {
		...
		for (Annotation sentence : sentences) {
			...
			java.util.List<Annotation> tokenslist = new java.util.LinkedList<gate.Annotation>(
					annotations.get("Token", sentence.getStartNode()
				...
				for (int i = 0; i < off.length; i++) {
					if (!off[i].equals("other")) {
						...
						try {
							...
							// get the class uri
							// String classUri =
							// type2uri.get(tr.get(tagger.name));
	...
	public Resource init() throws ResourceInstantiationException {
		...
		try {
			...
			throw new ResourceInstantiationException(
					"OpenNLP Name Finder could not be initialized", e);
		}
	...
	public static MaxentModel getModel(URL name) {
		try {
			...
			throw new RuntimeException(
					"OpenNLP NameFinder can not be initialized!", E);
		}
	}
