...
public class InflectionalGazetteer extends gate.creole.AbstractLanguageAnalyser
  ...
  /**
   * Gets the gaz reader.
   * @param file the gazetteer xml file
   * @return the gaz reader
   */
  private InflectionalGazetteerXMLReader getGazReader(URL url) throws ResourceInstantiationException {
    InflectionalGazetteerXMLReader mReader = new InflectionalGazetteerXMLReader(importOnlyTheseTypes);
    try {
      mReader.parse(url);
      return mReader;
    } catch(Exception e) {
      throw new ResourceInstantiationException("Error loading gazetteer from " + url, e);
    }
  } // getGazReader(url)
  ...
  public Resource init()throws ResourceInstantiationException{
    ...
    if(config == null){
      throw new ResourceInstantiationException (
          "No config provided for gazetteer!");
    } //if


    try {
      BufferedReader configReader = new BufferedReader(
          new InputStreamReader(config.openStream(), encoding));
      int lemmaIdx = 0;
      try {
  ...
  public void execute() throws ExecutionException{
    ...
    if ( DEBUG) {
      profiler = new Profiler();
      profiler.enableGCCalling(false);
      profiler.initRun("POS Tagger execute over document "+document.getName());
      profiler.checkPoint("reset");
   ...
   * @param lemma the lemma to be added
...
//   } // avoid endless recursion
  ...
  @CreoleParameter(comment = "Location of the configuration file",
      defaultValue = "resources/inflection_gaz/main.conf")
  ...
  @CreoleParameter(comment = "The encoding to use when reading configuration",
      defaultValue = "UTF-8")
  ...
  /** Find the list of mathcing feature maps and mainforms for the current phrase and map
   * @param phrase
   * @param map
