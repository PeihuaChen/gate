    ...
    implements RussIEConstants
{
    ...
    private void initMap()
    {
        ...
        for(StringTokenizer tok = new StringTokenizer(nnTypes, ","); tok.hasMoreElements(); categoriesMap.put(type, "NN"))
            type = tok.nextToken();

        fireStatusChanged("Categories in map count: " + categoriesMap.size());
        String nnsTypes = "Nfapa,Nfapd,Nfapg,Nfapi,Nfapl,Nfapn,Nfipa,Nfipd,Nfipg,Nfipi,Nfipl,Nfipn,Nmapa,Nmapd,Nmapg,Nmapi,Nmapl,Nmapn,Nmi2gpa,Nmi2gpd,Nmi2gpg,Nmi2gpi,Nmi2gpl,Nmi2gpn,Nmi2lgpa,Nmi2lgpd,Nmi2lgpg,Nmi2lgpi,Nmi2lgpl,Nmi2lgpn,Nmi2lpa,Nmi2lpd,Nmi2lpg,Nmi2lpi,Nmi2lpl,Nmi2lpn,Nmipa,Nmipd,Nmipg,Nmipi,Nmipl,Nmipn,Nnapa,Nnapd,Nnapg,Nnapi,Nnapl,Nnapn,Nnipa,Nnipd,Nnipg,Nnipi,Nnipl,Nnipn,Np,Nppa,Nppd,Nppg,Nppi,Nppl,Nppn";
        for(StringTokenizer tok = new StringTokenizer(nnsTypes, ","); tok.hasMoreElements(); categoriesMap.put(type, "NNS"))
            type = tok.nextToken();
        ...
        for(StringTokenizer tok = new StringTokenizer(nnpTypes, ","); tok.hasMoreElements(); categoriesMap.put(type, "NNP"))
            type = tok.nextToken();
        ...
        for(StringTokenizer tok = new StringTokenizer(nnpsTypes, ","); tok.hasMoreElements(); categoriesMap.put(type, "NNPS"))
            type = tok.nextToken();
        ...
        throws ExecutionException
    {
        ...
        if(inputASName == null || inputASName.equals(""))
            ...
            annotationSet = super.document.getAnnotations(inputASName);
    ...
    private void mapCategories(AnnotationSet annSet)
    {
        ...
        for(int index = 0; index < list.length;)
        {
            ...
            if(singleToken.size() == 0)
                showMessage("No token annotation for MSD annotation: " + msdAnn);
            else
                showMessage("No recognized category for MSD annotation: " + msdAnn);
            mappedTypes.clear();
        }
