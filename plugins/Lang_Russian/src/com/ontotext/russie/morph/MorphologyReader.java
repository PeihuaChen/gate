...
public class MorphologyReader implements RussIEConstants {
   ...
   * @param line a line from the Morphology
   * @return the constructed Lemma
   * @throws WrongFormatException*/
  private Lemma constructLemmaByMorphologyLine(String line) throws WrongFormatException{
...
//    ]).
    // check el prefix
    if (!line.substring(0,3).equals(LINE_PREFIX)) {
    ...
    if (commaIndex < 0 || nextCommaIndex < 0) {
    ...
    lemma.setMainForm(
        removeQuote(line.substring(3,commaIndex))
        ,type);

    if (!line.substring(nextCommaIndex+1,nextCommaIndex+2).equals("[")){
      throw new WrongFormatException();
    }
    ...
    lemma.getSuffixNest().setMainFormSuffix(lemma.getMainForm().substring(
        lemma.getRoot().length()));
    return lemma;
  ...
  /**
   * Removes the single quotes embracing a phrase.
   * Presumes that if there is a quote at the start
   * - there is also a quote at the end.
   * @param phrase
