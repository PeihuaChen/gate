 ...
 *
 * @todo: list of all types and of all nests, toString of lemma and nest.
 *
 *  This one is based on the RussGazetteer and performs POS Tagging without
 * disambiguation.
 *
 * <p>Title: RussIE</p>
...
public class POSTagger extends  gate.creole.AbstractLanguageAnalyser
  ...
  * @param file the morhology file
  * @return the morh reader
  */
 private MorphologyReader getMorphReader(URL url) {
   MorphologyReader mReader = new MorphologyReader(caseSensitive.booleanValue());
   ...
   try {
     ...
     throw new GateRuntimeException(
         "Loading of the morphology failed.");
   }
   return mReader;
 } // getMorphReader(url)
...
//   } // avoid endless recursion
 ...
 @CreoleParameter(comment = "Location of configuration file listing "
     + "morphology files to be loaded",
     defaultValue = "resources/morphology/main.conf")
...
public URL getConfig() {
 ...
 /** Find the list of mathcing types and mainforms for the current phrase and map
  * @param phrase
  * @param map
       ...
       while (ni.hasNext()){
         ...
         if (nest != null) {
           ...
           if ( newTypes != null) {
             ...
             if (DETAILED_DEBUG) {
               System.out.println("\nUNRECOGNIZED PHRASE - > "+phrase+suffix);
 ...
 @CreoleParameter(comment = "Should this POS Tagger differentiate on case",
     defaultValue = "true")
