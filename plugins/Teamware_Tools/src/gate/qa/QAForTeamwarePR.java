...
public class QAForTeamwarePR extends AbstractLanguageAnalyser implements
  ...
  public Resource init() throws ResourceInstantiationException {
    ...
    assurancePR =
      (QualityAssurancePR)Factory.createResource("gate.qa.QualityAssurancePR",
  ...
  public void execute() throws ExecutionException {
    ...
    if(measure == null) { throw new ExecutionException("No measure selected"); }
    ...
    consensusExists =
      (consensusExists || document.getNamedAnnotationSets().containsKey(
        CONSENSUS_AS_NAME));
    ...
    for(String annName : annotatorToDocuments.keySet()) {
      ...
      for(Object aDoc : docs) {
        ...
        if(!(aDoc instanceof Document)) {
          try {
            gateDoc =
              (Document)getCorpus().getDataStore().getLr(
    ...
    for(String annotatorsPair : annotatorsPairToDocuments.keySet()) {
      ...
      String annotator1 =
        annotatorsPair.substring(0, annotatorsPair.indexOf(';'));
      String annotator2 =
        annotatorsPair.substring(annotatorsPair.indexOf(';') + 1);
      // one doc at a time
      for(Object aDoc : docs) {
    ...
    File[] resultFiles =
      new File(outputFolderUrl.getFile()).listFiles(new FileFilter() {
        public boolean accept(File pathname) {
          return pathname.getAbsolutePath().endsWith("-document-stats.html");
        }
      });
    ...
    for(File file : resultFiles) {
      ...
      List<Annotation> cols =
        new ArrayList<Annotation>(omSet.getContained(Utils.start(row),
          Utils.end(row)).get("td"));
      Collections.sort(cols, new OffsetComparator());
      // only interested in the last column
      r.micro =
        Double.parseDouble(Utils.stringFor(aDoc, cols.get(cols.size() - 1)));
    ...
    for(String author1 : authorNamesList) {
      ...
      if(author1.equals(CONSENSUS_AS_NAME)) {
        double caMacroAvg =
          (double)(consensusMacro / (double)(columnAuthorNames.size() - 1));
        double caMicroAvg =
          (double)(consensusMicro / (double)(columnAuthorNames.size() - 1));
    ...
    buffer.append("<hr>");
    if(consensusExists) {
      buffer.append("<br><b>Avg. " + CONSENSUS_AS_NAME + " macro avg:</b> " +
        (f.format((double)consensusMacro / (columnAuthorNames.size() - 1))));
      buffer.append("<br><b>Avg. " + CONSENSUS_AS_NAME + " micro avg:</b> " +
        (f.format((double)consensusMicro / (columnAuthorNames.size() - 1))));
    }
    // if consensus annotation set exists, make sure the number used in
    // demoninator does not include consensus as one annotator
    int totalAuthors =
      consensusExists ? columnAuthorNames.size() - 1 : columnAuthorNames.size();
  ...
  /**
   * A method that adds two columns to the buffer - one for the macro figure and
   * the other one for the micro figure. It also adds relevant style tag to give
   * a proper color to each cell depending on the value of macro and micro
   * figures. See documentation of the getStyleTag(double) for more information
   * on how cell backgrounds are color coded.
   * 
   * @param buffer
   * @param macro
   * @param micro
   ...
   * becomes lighter as the agreement reduces towards 0.5. At 0.5 agreement, the
   * background color of a cell is fully white. From 0.5 downwards, the color
   * red is used and as the agreement reduces further, the color becomes darker
   * with dark red at 0.0 agreement.
   * 
   * @param score
   * @return
   */
  private String getStyleTag(double score) {
    ...
    if(score > 0.50) {
      ...
      c =
        new Color(rc.getRed(), rc.getGreen(), rc.getBlue(),
          100 - ((int)(score * 100) * 2));
    }
  ...
  private void createAnnSet(Document doc, String inputAS, String outputAS) {
    AnnotationSet inAS =
      inputAS == null || inputAS.trim().length() == 0
        ? doc.getAnnotations()
  ...
  private void calculateIAA(Corpus corpus, String keyAS, String responseAS,
    ...
    corpusFile.delete();
    File documentFile =
      new File(outputFolderUrl.getFile(), "document-stats.html");
    File documentOutFile =
      new File(outputFolderUrl.getFile(), filePrefix + "-document-stats.html");
