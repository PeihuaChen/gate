...
public class LearningAPIMain extends AbstractLanguageAnalyser implements
  ...
  /**
   * The lightweight learning object for getting the features, training
   * and application.
   */
  ...
  public gate.Resource init() throws ResourceInstantiationException {
    ...
    if(!wd.isDirectory()) {
      throw new ResourceInstantiationException(wd
    ...
    if(positionDoc == corpus.size() - 1) {
      ...
      try {
        ...
        if(learningSettings.isOnlyFeatureData) {// if only want feature
          ...
          if(!trainingMode && !evaluationMode) { // if application
  ...
  public Boolean getTrainingMode() {
    return new Boolean(this.trainingMode);
  }

  public void setTrainingMode(Boolean trainingMode) {
    this.trainingMode = trainingMode.booleanValue();
  }

  public Boolean getEvaluationMode() {
    return new Boolean(this.evaluationMode);
  }

  public void setEvaluationMode(Boolean evaluationMode) {
    this.evaluationMode = evaluationMode.booleanValue();
  }
