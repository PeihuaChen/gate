...
public class EvaluationBasedOnDocs {
  ...
  HashMap labels2MMR = new HashMap(); // label to macroMeasure result of
                                      // the label

  // Label to number of the runs having the label, used for
  // macro-averaged for each label
  ...
  public void kfoldEval(LearningEngineSettings learningSettings,
          LightWeightLearningApi lightWeightApi, PrintWriter logFileIn)
          throws GateException {
    int k = learningSettings.evaluationconfig.k;
    ...
    for(int nr = 0; nr < k; ++nr) {
      ...
      HashMap labels2MR = new HashMap(); // label to measure of result
                                          // of the label
  ...
  public void holdoutEval(LearningEngineSettings learningSettings,
    ...
    int trainingNum = (new Double(Math
            .floor((numDoc * learningSettings.evaluationconfig.ratio)))
            .intValue());
    ...
    for(int nr = 0; nr < k; ++nr) {
      ...
      for(int i = 0; i < testNum; ++i) {
        ...
        if(isDuplicate) {// If cannot select a non-duplicate index in
        ...
        // System.out.println("i="+new Integer(i)+".");
  ...
  private void oneRun(LearningEngineSettings learningSettings,
    ...
    if(learningSettings.datasetDefinition.dataType == DataSetDefinition.RelationData) {
      ...
      for(int i = 0; i < corpusOn.size(); ++i) {
        if(!isUsedForTraining[i]) {
          evaluateAnnotationsRel((Document)corpusOn.get(i), classTypeOriginal,
                  classTypeTest, classFeature, arg1F, arg2F, uniqueLabels
                          .keySet(), labels2MR);
        }
      }
    }
      ...
      /*
       * for(int i=0; i<corpusOn.size(); ++i) { if(!
       * isUsedForTraining[i]) {
       * evaluateAnnotations((Document)corpusOn.get(i),
       * classTypeOriginal, classTypeTest, classFeature,
       * uniqueLabels.keySet(), labels2MR); } }
       */

      // evaluation on each label
      for(int i = 0; i < corpusOn.size(); ++i)
        if(!isUsedForTraining[i]) {
    ...
    // finally, change the class type back for training, and remove the
    // test
    // application
    learningSettings.datasetDefinition.getClassAttribute().setType(
            classTypeOriginal);
  ...
  public void add2MacroMeasure(EvaluationMeasuresComputation measuresOfResults,
          HashMap labels2MR,
          EvaluationMeasuresComputation macroMeasuresOfResults,
  ...
  public void evaluateAnnDiff(Document doc, String classTypeOriginal,
          String classTypeTest, String classFeat, Set labelSet,
          HashMap labels2MR) {
    // for each label
  ...
  /**
   * Evaluate the test document by comparing the annotations with the
   * golden standard
   */
  ...
  /**
   * Evaluate the test document by comparing the annotations with the
   * golden standard, for Relation learning
   */
