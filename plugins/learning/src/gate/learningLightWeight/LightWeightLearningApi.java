...
import gate.learningLightWeight.learners.ChunkOrEntity;
...
public class LightWeightLearningApi extends Object {
  ...
  /**
   * The annotationSet that contains annotations to be considered in
   * dataset
   */
  ...
  BufferedReader inFVList = null;

  public LightWeightLearningApi(File wd) {

    try {
      this.workingDirectory = wd.toURL();
    }
  ...
  /** Further initialisation for learning. */
  ...
  /**
   * Obtain the features and labels and form feature vectors from the
   * GATE annotation of each document.
   */
  public void annotations2FVs(Document doc, int numDocs, File wdResults,
    ...
    docFV.obtainFVsFromNLPFeatures(nlpFeaturesDoc, featuresList,
            featurePositionTotal, maxNegPositionTotal,
            featuresList.totalNumDocs);
  ...
  /**
   * The works when finishing the conversion from annotations to feature
   * vectors.
   */
  ...
  void addDocFVsMultiLabelToFile(int index, BufferedWriter out,
    ...
    try {
      out.write(new Integer(index) + ConstantParameters.ITEMSEPARATOR
              + new Integer(docFV.numInstances)
              + ConstantParameters.ITEMSEPARATOR + docFV.docId);
      out.newLine();
      for(int i = 0; i < docFV.numInstances; ++i) {
        StringBuffer line = new StringBuffer();
        ...
        for(int j = 0; j < docFV.fvs[i].len; ++j)
          line.append(ConstantParameters.ITEMSEPARATOR
                  + docFV.fvs[i].indexes[j]
                  + ConstantParameters.INDEXVALUESEPARATOR
                  + docFV.fvs[i].values[j]);
  ...
  public void trainingJava(int numDocs, PrintWriter logFileIn,
    ...
    File wd = new File(URI.create(workingDirectory.toExternalForm()));
    ...
    int learnerType = obtainLearnerType(engineSettings.learners.learnerName);

    switch(learnerType) {
      case 1: // for weka learner
        logFileIn.println("Use weka learner.");
        WekaLearning wekaL = new WekaLearning();
        short featureType = obtainWekaLeanerDataType(engineSettings.learners.learnerName);
        // Convert and read training data
        switch(featureType) {
          case WekaLearning.NLPFEATUREFVDATA:
            // Transfer the labels in nlpDataFile into the label in the
            // sparse data
        ...
        WekaLearner wekaLearner = obtainWekaLearner(engineSettings.learners.learnerName);
      ...
      case 2: // for learner of multi to binary conversion
        ...
        // get a learner
        // String learningCommand = engineSettings.learningExecutable
        // + " " + engineSettings.paramsOfLearning;
        String learningCommand = engineSettings.paramsOfLearning;
        ...
        SupervisedLearner paumLearner = MultiClassLearning
                .obtainLearnerFromName(engineSettings.learners.learnerName,
                        learningCommand, dataSetFile);
        paumLearner.setLearnerExecutable(engineSettings.learningExecutable);
        paumLearner.setLearnerParams(engineSettings.paramsOfLearning);
  ...
  public void applyModelInJava(Corpus corpus, String labelName,
    ...
    File nlpDataFile = new File(nlpFileName);
    File modelFile = new File(modelFileName);
    File labelInDataFile = new File(labelInDataFileName);

    int learnerType;
    learnerType = obtainLearnerType(engineSettings.learners.learnerName);
    ...
    switch(learnerType) {
      case 1: // for weka learner
        ...
        // features
        featureType = obtainWekaLeanerDataType(engineSettings.learners.learnerName);
        ...
        boolean distributionOutput = obtainWekaLearnerOutputType(engineSettings.learners.learnerName);
        // Get the wekaLearner from the learnername
        WekaLearner wekaLearner = obtainWekaLearner(engineSettings.learners.learnerName);
        logFileIn.println("Weka learner name: " + wekaLearner.getLearnerName());
        // Training.
  ...
  private void addAnnsInDoc(Document doc, HashSet chunks, String instanceType,
    ...
    for(Object obj : chunks) {
      ...
      // ChunkOrEntity entity =
      // (ChunkOrEntity)mapChunks.get(indexes.get(i));
  ...
  public int obtainLearnerType(String learnerName) throws GateException {
    if(learnerName.equals("SVMLibSvmJava") || learnerName.equals("C4.5Weka")
            || learnerName.equals("KNNWeka")
            || learnerName.equals("NaiveBayesWeka")) {
  ...
  public short obtainWekaLeanerDataType(String learnerName) {
    if(learnerName.contains("C4.5") || learnerName.contains("NaiveBayes")) {
      return WekaLearning.NLPFEATUREFVDATA;
    }
    else {
      return WekaLearning.SPARSEFVDATA;
    }
  }

  public WekaLearner obtainWekaLearner(String learnerName) {
  ...
  public boolean obtainWekaLearnerOutputType(String learnerName) {
    /*
  ...
  public void convert3labelsT2label(LabelsOfFeatureVectorDoc[] labelsFVDocWeka) {
    // For each document
    for(int iDoc = 0; iDoc < labelsFVDocWeka.length; ++iDoc) {
      // For each instance
      for(int i = 0; i < labelsFVDocWeka[iDoc].multiLabels.length; ++i) {
        int totalNum = labelsFVDocWeka[iDoc].multiLabels[i].num / 3 * 2;
        float[] newLabelProbs = new float[totalNum];
        for(int j = 0; j < labelsFVDocWeka[iDoc].multiLabels[i].num; j += 3) {
          // Choose the higher one
          if(labelsFVDocWeka[iDoc].multiLabels[i].probs[j] > labelsFVDocWeka[iDoc].multiLabels[i].probs[j + 2])
            newLabelProbs[j / 3] = labelsFVDocWeka[iDoc].multiLabels[i].probs[j];
          else newLabelProbs[j / 3] = labelsFVDocWeka[iDoc].multiLabels[i].probs[j + 2];
          if(labelsFVDocWeka[iDoc].multiLabels[i].probs[j + 1] > labelsFVDocWeka[iDoc].multiLabels[i].probs[j + 2])
            newLabelProbs[j / 3 + 1] = labelsFVDocWeka[iDoc].multiLabels[i].probs[j + 1];
          else newLabelProbs[j / 3 + 1] = labelsFVDocWeka[iDoc].multiLabels[i].probs[j + 2];
        }
        // Use the new 2 label probabilities
        labelsFVDocWeka[iDoc].multiLabels[i] = new LabelsOfFV(totalNum);
        labelsFVDocWeka[iDoc].multiLabels[i].probs = newLabelProbs;
      }
    }
  }

  /** flitering the training data using the SVM */
  public void FilteringNegativeInstsInJava(int numDocs, PrintWriter logFileIn,
    ...
    // If want to remove the negative which are close to positive one,
    // reverse the scores.
        ...
        for(int j = 0; j < labelsFVDocB[i].multiLabels.length; ++j) {
          if(labelsFVDocB[i].multiLabels[j].num > 0
            ...
            for(int j1 = 0; j1 < labelsFVDocB[i].multiLabels[j].num; ++j1) {
              // ("i="+i+", j="+j+",
              // num="+labelsFVDocB[i].multiLabels[j].num+",
              // j1="+j1+".");
              line.append(ConstantParameters.ITEMSEPARATOR
