...
public class OntologyTreeListener extends MouseAdapter {
	...
	/**
	 * Indicates if it is a new annotation mode. IF this is set to true, the
	 * selected text is annotated with the user specified class in the ontology
	 * tree (user selects a class by clicking on its name)
	 */
	protected boolean newAnnotationMode = false;
	...
	/**
	 * This method is invoked whenever user clicks on one of the classes in the ontology tree
	 */
	public void mouseClicked(MouseEvent me) {
		...
		if (path != null) {
			final IFolder node = (IFolder) path.getLastPathComponent();
			// ok let us see if this was a right click
			if (SwingUtilities.isRightMouseButton(me)) {
				// it is a right click
				final Color color = ontologyTreePanel.currentClass2ColorMap
						.get(node.toString());
				...
				changeColor.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent ae) {
						Color col = JColorChooser
								.showDialog(ontologyTreePanel,
										"Select colour for \""
												+ node.toString() + "\"", color);
						if (col != null) {
							Color colAlpha = new Color(col.getRed(), col
									.getGreen(), col.getBlue(), 128);
							ontologyTreePanel.setColor(node.toString(),
									colAlpha);
							// so let us update our tree
							// and rehighlight
							ontologyTreePanel.repaint();
							ontologyTreePanel.ontoTreeListener
									.refreshHighlights();
			...
			// if the newannotationmode is true that means we want to add a new
			// annotation
			if (newAnnotationMode) {
				newAnnotationMode = false;
				ontologyTreePanel.ontoViewer.annotationAction.hideAllWindows();
				ontologyTreePanel.ontoViewer.annotationAction
						.setNewAnnotationWindowShowing(false);
				addNewAnnotation(node.toString(), false, null);
				return;
			}

			boolean isSelected = !ontologyTreePanel.currentClass2IsSelectedMap
	...
	private void expandChildren(Object[] paths, IFolder node) {
		...
		if (node.getChildCount() > 0) {
			...
			if (!ontologyTreePanel.currentOntologyTree.isVisible(temp)) {
				ontologyTreePanel.currentOntologyTree.expandPath(new TreePath(
						myArray));
			}
		}
	}
	...
	/**
	 * This method is to find out the path for this node from the root of the
	 * tree
	 * 
	 * @param node
	 * @return
	...
	public TreePath getTreePath(IFolder node) {
		IFolder root = (IFolder) ontologyTreePanel.currentOntologyTree
				.getModel().getRoot();
	 ...
	 * @return
	 */
	private Object[] traverseThroughPath(IFolder currentNode,
			IFolder nodeToFind, Object[] path) {
		...
		if (currentNode.getChildCount() > 0) {
			...
			while (children.hasNext()) {
				...
				Object[] returnedPath = traverseThroughPath(node, nodeToFind,
						tempPath);
	...
	/**
	 * This method finds out the IFolder node in the ontology Tree for given
	 * class
	 * 
	 * @param classValue
	 * @return
	 */
	public IFolder getNode(String classValue) {
		JTree tree = ontologyTreePanel.currentOntologyTree;
		IFolder currentNode = (IFolder) tree.getModel().getRoot();
		return getClassNode(currentNode, classValue);
	}

	/**
	 * Internal recursive method to find out the Node for given class Value
	 * under the heirarchy of given node
	 * 
	 * @param node
	 * @param classValue
	 * @return
	 */
	private IFolder getClassNode(IFolder node, String classValue) {
		...
		while (children.hasNext()) {
			IFolder tempNode = (IFolder) children.next();
			IFolder returnedNode = getClassNode(tempNode, classValue);
	 ...
	 * @param all
	 */
	public ArrayList<Annotation> addNewAnnotation(String classValue,
			boolean all, FeatureMap map) {
		...
		int start = ontologyTreePanel.ontoViewer.documentTextArea
				.getSelectionStart();
		int end = ontologyTreePanel.ontoViewer.documentTextArea
				.getSelectionEnd();
		...
		// so now we have annotation offsets in offsets arrayList
		IFolder node = getNode(classValue);
		...
		map.put(gate.creole.ANNIEConstants.LOOKUP_CLASS_FEATURE_NAME,
				classValue);
		String dns = ontologyTreePanel.getCurrentOntology()
				.getDefaultNameSpace();
		...
		for (int i = 0; i < offsets.size(); i++) {
			...
			try {
				...
				if (i == 0) {
					ontologyTreePanel.setSelected(classValue, true);
					// and finally we need to expand the path
					if (node != null) {
						TreePath path = getTreePath(node);

						if (!ontologyTreePanel.currentOntologyTree
								.isVisible(path)) {
							ontologyTreePanel.currentOntologyTree
									.expandPath(path);
						}
					}
				}
	...
	public void refreshHighlights() {
		...
		HashMap<String, ArrayList<Annotation>> currentClassName2AnnotationsListMap = ontologyTreePanel.currentClassName2AnnotationsListMap;
		if (currentClassName2AnnotationsListMap == null)
			return;
			...
			if (annotationsList == null || annotationsList.isEmpty()) {
				...
				for (int j = 0; j < annotationsList.size(); j++) {
					...
					if (ann.getType().equals(typeToMatch)) {
						String set = ontologyTreePanel.ontoViewer
								.getAnnotationSet(ann);
						if (set == null
								&& setToMatch
					...
					try {
						Color color = ontologyTreePanel
								.getHighlightColor(className);
						Object tag = highlighter.addHighlight(ann
								.getStartNode().getOffset().intValue(), ann
		...
		// This is to make process faster.. instead of accessing each annotation
		// and its offset, we create an array with its annotation offsets to search
		// faster
