...
import shef.nlp.buchart.utils.SynOutput;
import shef.nlp.buchart.utils.SemOutput;
import shef.nlp.buchart.prolog.*;


public class Buchart  extends AbstractLanguageAnalyser implements ProcessingResource, Serializable
{
   /** Name of the temp file prefix for the input buchart **/
   protected static String InTempFileName = "BuChart--IN--";

   /** Name of the temp file prefix for the output from buchart  **/
   protected static String OutTempFileName = "BuChart--OUT--";

   /** Name of the temp file prefix for the semantic output from buchart **/
   protected static String SemTempFileName = "BuChart--SEM--";

   public File InTempFile, OutTempFile, SemTempFile;


   /** The name of the executable BuChart **/
   public File buchartFile;
   public void setBuchartFile(File buchartFile) { this.buchartFile = buchartFile; }
   public File getBuchartFile() { return buchartFile; }
...
*/
   ...
   public ArrayList buchartConstraints;
   public ArrayList buchartVariables;
   ...
   public Hashtable buchartCategories;
   ...
   public static final String ATT_DELIMITER=";";
   public static final String GATE_LINE="Gate";
   public static final String BUCHART_LINE="Buchart";
   public static final String BUCHART_CAT="category";
         ...
         * **/
         while((line = in.readLine()) != null)
         {
            ...
            if(line.startsWith(GATE_LINE))
            {
               ...
               line = in.readLine();
               lnum++;
               if(line.startsWith(BUCHART_LINE))
               {
                  constraints=Factory.newFeatureMap();
                  variables=Factory.newFeatureMap();
   ...
   public void execute() throws ExecutionException
   {
      ...
      for(int s=0;s<sentList.size();s++)
      {
         ...
         charts[s]=new Chart(new Long(s+1),startSent,endSent,new BuChartRecord("top",startSent,startSent,Factory.newFeatureMap()));
         ...
         for(int a=0;a<inSentenceList.size();a++)
         {
            ...
            if(index>=0)
            {
               ...
               record=new BuChartRecord((String) buchartMap.get("category"),annStart,annEnd,buchartMap);
         ...
         Collections.sort(buchartList,BuChartRecord.BuChartRecordComparator());
      ...
      else
      {
         ...
         {
            record=(BuChartRecord) workingList.get(c);
         ...
         charts[s].setFinal(new BuChartRecord("bottom",endSent,endSent,Factory.newFeatureMap()));


         out.println(charts[s].toBuchartFormat(buchartCategories));
         out.flush();
      }
   ...
   {
      ArrayList working=new ArrayList();
      BuChartRecord current;
      BuChartRecord previous;
      long presize;
      long cursize;
      Long prestart, preend, start, end;
      int index=0;
      ...
      if(list.size()>0)
      {
         previous=(BuChartRecord)list.get(0);
         ...
         for(int e=1;e<list.size();e++)
         {
            current=(BuChartRecord) list.get(e);
            ...
            previous=(BuChartRecord)working.get(working.size()-1);
            prepriority=((Integer) priorities.get(previous.getCategory())).intValue();
            prestart=previous.getStart();
            preend=previous.getEnd();
            presize=preend.longValue()-prestart.longValue();
