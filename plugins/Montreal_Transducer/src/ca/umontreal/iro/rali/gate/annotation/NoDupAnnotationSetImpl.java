...
/**
  ...
  When constructing a new object from another AnnotationSet object, duplicates are
  copied as is.  Only additions made afterwards are subject to duplicate control.
*/
...
public class NoDupAnnotationSetImpl extends gate.annotation.AnnotationSetImpl {
  ...
  /** Construction from Collection (whose items must be Annotation).  Do not
   check for duplicate annotations: copy them as is. */

  public NoDupAnnotationSetImpl(Collection c) throws ClassCastException {
    super(c);
    authoriseDuplicates = Boolean.FALSE;
  }
  ...
  public boolean add(Annotation a) throws ClassCastException {
	...
	if (subsubset != null && !subsubset.isEmpty()) {
	  ...
	  for (Iterator iter=subsubset.iterator(); iter.hasNext();) {
	    if (get(((Annotation) iter.next()).getId()) != null) {
	      return false;
	    }
	  }
	}
 ...
 /**
   ...
   * @param authoriseDuplicates value of authoriseDuplicates.
   */
